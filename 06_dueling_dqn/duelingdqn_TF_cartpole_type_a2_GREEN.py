...
# [Dueling DQN] Sparate main net as state-net and action-net
...
LOG_DIR_Model = "/tmp/RL/06_Dueling_DQN/Model"
LOG_DIR_Graph = "/tmp/RL/06_Dueling_DQN/Graph"
    ...
    for episode in range(N_EPISODES):
        ...
        if episode % TARGET_UPDATE_CYCLE == 0 and len(replay_buffer) > MINIBATCH:
            for sample in ran.sample(replay_buffer, MINIBATCH):
                ...
                #           y_j = r_j + gamma * max_a' Q^(Pi_j+1, a'; 𝜃~)   for non-terminal Pi_j+1
                if done_R_M:

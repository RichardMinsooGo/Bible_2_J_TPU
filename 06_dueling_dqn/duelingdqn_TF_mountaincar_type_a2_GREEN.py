...
class DQN:
    ...
    def build_model(self, H_SIZE_01 = 256, H_SIZE_02_state = 256, H_SIZE_02_action = 256, Alpha=0.001) -> None:
        with tf.variable_scope(self.net_name):
            ...
            net_state = tf.layers.dense(net_0, H_SIZE_02_state, activation=tf.nn.relu)
            net_action = tf.layers.dense(net_0, H_SIZE_02_action, activation=tf.nn.relu)

            net_16_state = tf.layers.dense(net_state, H_SIZE_16_state)
            net_16_action = tf.layers.dense(net_action, H_SIZE_16_action)

    ...
    while time.time() < timeout_start + timeout:        
        ...
        if (episode+1) % TARGET_UPDATE_CYCLE == 0 and len(replay_buffer) > MINIBATCH:
            for _ in range (MINIBATCH):
                for sample in ran.sample(replay_buffer, 10):
            ...
            print("Episode {:>5} reward:{:>5} average reward:{:>5.2f} recent N Game reward:{:>5.2f} Loss:{:>5.2f} memory length:{:>5}"
                  .format((episode+1), rall, np.mean(rlist), np.mean(last_N_game_reward),loss,len(replay_buffer)))
...
with tf.Session() as sess:
    ...
    for episode in range(N_train_result_replay):
        ...
        rlist.append(rall)
        print("Episode : {:>5} rewards ={:>5}. averge reward : {:>5.2f}".format((episode+1), rall,
                                                                        np.mean(rlist)))

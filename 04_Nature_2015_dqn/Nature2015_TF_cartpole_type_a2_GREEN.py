...
LOG_DIR_Model = "/tmp/RL/04_Nature2015/Model"
LOG_DIR_Graph = "/tmp/RL/04_Nature2015/Graph"
...
with tf.Session() as sess:
    ...
    # 15.	Initialize target action-value function Q^ with weight 𝜃~ = 𝜃
    ...
    for episode in range(N_EPISODES):
        ...
        if episode % TARGET_UPDATE_CYCLE == 0 and len(replay_buffer) > MINIBATCH:
            for sample in ran.sample(replay_buffer, MINIBATCH):
                ...
                #           y_j = r_j + gamma * max_a' Q^(Pi_j+1, a'; 𝜃_target)   for non-terminal Pi_j+1
                if done_R_M:
            ...
            # 17.4.	Every C steps reset Q^ = Q
            # 업데이트 사이클 마다 target 네트워크에 main 네트워크 값을 copy
            # Copy the weights theta from main network to target network
            sess.run(W01_t.assign(W01_m))
            sess.run(W16_t.assign(W16_m))
            sess.run(B01_t.assign(B01_m))

...
W01_t = tf.get_variable('W01_t',shape=[state_size, H_SIZE_01])
W16_t = tf.get_variable('W16_t',shape=[H_SIZE_01, action_size])
...
optimizer = tf.train.AdamOptimizer(learining_rate, epsilon=0.01)
train = optimizer.minimize(LossValue)

model_path = "/tmp/RL/04_Nature2015/Model"
graph_path = "/tmp/RL/04_Nature2015/Graph"
        ...
        if len(replay_buffer) > MINIBATCH:
            ...
            for sample in ran.sample(replay_buffer, MINIBATCH):
                ...
                if done_R_M:
                    ...
                    nextstate_reshape_R_M = np.reshape(nextstate_R_M,[1,state_size])
                    Q_target = sess.run(Qpred_t, feed_dict={X: nextstate_reshape_R_M, dropout:1})
                    Q_Global[0, action_R_M] = reward_R_M + discount_factor * np.max(Q_target)

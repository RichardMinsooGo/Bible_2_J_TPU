...
W15_t = tf.get_variable('W15_t',shape=[H_SIZE_14, H_SIZE_15])

W16_t = tf.get_variable('W16_t',shape=[H_SIZE_15, OUTPUT_SIZE])
...
B15_t = tf.Variable(tf.zeros([1],dtype=tf.float32))
...
LAY15_t = tf.nn.relu(tf.matmul(LAY14_t ,W15_t)+B15_t)

Qpred_t = tf.matmul(LAY15_t,W16_t)
        ...
        if episode % TARGET_UPDATE_CYCLE == 0 and len(replay_buffer) > MINIBATCH:
            ...
            for sample in ran.sample(replay_buffer, MINIBATCH):
                ...
                if done_R_M:
                    ...
                    Q_target, Q_m = sess.run([Qpred_t,Qpred_m], feed_dict={X: nextstate_reshape_R_M})
